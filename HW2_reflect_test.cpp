...
const double VIEW_D = 1.0; // arbitrary, not affecting output results
...
unordered_map<string, vector<double>> Chk_Sphere(vector<double> light_pos, vector<double> eye_pos,
    ...
    if(det <= radius){
        ...
        vector<double> reflect_rate = {mat[7]};
        res["reflect_rate"] = reflect_rate;
...
unordered_map<string, vector<double>> Chk_Triangle(vector<double> light_pos, vector<double> eye_pos,
    ...
    for(int i=0;i<tri_num;i++){ // i: index of the triangle.
        // material info
        vector<double> mat = material[tri_p[i].back()-1]; // material info with mat No. in tri
        ...
        if(coef[2]<=0){
            ...
            if(1>=coef[0]>=0 && 1>=coef[1]>=0){
                if(coef[0]+coef[1] <= 1 ){
                    ...
                    // record which triangle No. is
                    // careful with the multiple intersections with triangles
                    vector<double> tri_no{i*1.0};
                    res["tri_no"] = tri_no;

                    // record reflection info
                    vector<double> reflect_rate{mat[7]};
                    res["reflect_rate"] = reflect_rate;
...
int main(){
    ...
    vector<double> sphere; // centre position, radius
    ...
    if (Input_File.is_open()){
        ...
        while (getline(Input_File, line)) {
            ...
            while(str2value >> str_value){ //read every word, delimited by space
                ...
                switch(header){
                    ...
                    case 'M': // mats
                        ...
                        break;
                    case 'S': // sphere
                        sphere.push_back(stof(str_value));
                        break;
                    case 'T': // triangles
            ...
            if(header == 'S'){
                sphere.push_back(mat_no); // record texture No.
    ...
    */
    ...
    for(int row=HEIGHT;row>0;row--){
        for(int col=WIDTH;col>0;col--){
            ...
            ray.reserve(3);
            // reflect vars
            unordered_map<string, vector<double>> reflect_res;
            vector<double> intersection_pos;
            vector<double> reflect_ray;
            vector<double> reflect_color;
            intersection_pos.reserve(3);
            reflect_ray.reserve(3);
            reflect_color.reserve(3);
            ...
            res = Chk_Sphere(light_pos, eye_pos, ray, sphere, tri_p, mat, is_sphere);
            pixel_color = res["color"];

            // check global reflection with other objects
            reflect_ray = res["reflection"];
            intersection_pos = res["intersection"]; // pos on sphere
            if(!is_sphere){
                res = Chk_Triangle(light_pos, eye_pos, ray, sphere, tri_p, mat);
                intersection_pos = res["intersection"]; // pos on triangle
                reflect_res = Chk_Sphere(light_pos, intersection_pos, reflect_ray, sphere, tri_p, mat, is_sphere);
                if(is_sphere){
                    double reflect_rate = reflect_res["reflect_rate"][0];
                    if(reflect_rate!=0.0){
                        reflect_color = vec_scalar(reflect_rate, reflect_res["color"]);
                        res["color"] = vec_sum(pixel_color, reflect_color);
                        // update pixel color
                        pixel_color = res["color"];
                    }
                }
            ...
            else{ // chk sphere's reflection from tri
                // tracing once in this case.
                reflect_res = Chk_Triangle(light_pos, intersection_pos,
                                            reflect_ray, sphere, tri_p, mat);
                if(reflect_res["color"]!=BLACK){ // not accurate detection of a reflection from object
                    double reflect_rate = reflect_res["reflect_rate"][0];
                    if(reflect_rate!=0.0){
                        reflect_color = vec_scalar(reflect_rate, reflect_res["color"]);
                        res["color"] = vec_sum(pixel_color, reflect_color);
