...
const double VIEW_D = 60.0; // the distance from eye to the screen
                            // arbitrary, not affecting output results, when not considering depth
const double APERTURE = 15/2.2; // apeRture size (diameter in mm)
...
//================================ Ray Tracing =====================================
    ...
    for(int row=HEIGHT;row>0;row--){
        for(int col=WIDTH;col>0;col--){
            ...
            const int EYE_SAMPLE_N = 10; // number of samples of eye distributed position
            // distributed ray tracing, i.e. distributed eye positions in apeRture range
            vector<double> r_arr{};
            vector<double> g_arr{};
            vector<double> b_arr{};
            ...
            for(size_t n{0}; n < EYE_SAMPLE_N; n++){
                ...
                double rand_r{(APERTURE/20)*uni_f(re)}; // notice: apeRture unit is mm, convert to unit:cm here.
